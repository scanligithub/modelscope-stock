...
def fetch_sina_flow(code, start, end, cf_url):
    symbol = code.replace(".", "")
    if not cf_url: return pd.DataFrame()
    worker_url = f"https://{cf_url}" if not cf_url.startswith("http") else cf_url
    params = {
        "target_func": "sina_flow", "page": 1, "num": 10000,
...
def run_stock_pipeline(year=0):
    ...
    print("🔍 正在寻找最近可用的股票列表...")
    for i in range(10):
        target_d = (datetime.datetime.now() - datetime.timedelta(days=i)).strftime("%Y-%m-%d")
        rs = bs.query_all_stock(day=target_d)
        if rs.error_code == '0' and len(rs.data) > 0:
            while rs.next():
                data.append(rs.get_row_data())
            print(f"✅ 成功获取日期为 {target_d} 的股票清单，共 {len(data)} 条。")
            break
    ...
    if not data:
        print("❌ 严重错误：无法从 Baostock 获取股票列表，请检查网络或 Baostock 服务状态。")
        return

    valid_codes = [x[0] for x in data if x[0].startswith(('sh.', 'sz.', 'bj.')) and x[2].strip()]
    print(f"🚀 开始采集 {len(valid_codes)} 只股票的历史数据 ({start} 至 {end})...")
    ...
    if res_k: 
        ...
        print(f"📦 K线合并完成，共 {len(df_k_all)} 行。")
    ...
    if res_f: 
        ...
        print(f"📦 资金流合并完成，共 {len(df_f_all)} 行。")